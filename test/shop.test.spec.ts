...
describe('GET /:filter/:limit?/:offset?', () => {
    ...
    it('should get products by filter text with bigger limit than the filter result based on offset', (done) => {
        const app = express();
        app.use('/api', routes);
        chai.request(app.listen(3000)).get(`/api/shop/cleaner a/10/4`)
            .end((err, res) => {
                if (err) return done(err);
                res.should.have.status(200);
                res.body.should.be.a('array');
